...
export abstract class BrokerComponent {
  ...
  constructor(props: {
    ...
    clone?: {
      aBrokerComponent: BrokerComponent;
      aName: string;
    };
  }) {
  ...
  abstract setup(onSetupFinish: FutureArbFnc): Promise<void>;

  private async onChannelOpened(channel: Channel) {
    this.setChannel(channel);
    try {
      await this.setup(this.onSetupFinish);
    } catch (error) {
      this.handleError(error);
    }
  }
